...
public class PostRepository : IPostRepository
{
    ...
    public async Task<IEnumerable<Post>> GetAllPosts(PostGetAllRequestDto postGetAllRequest)
    {
        ...
        if (postGetAllRequest.SearchTerm is not null)
            query = query.Where(p => p.Title.ToLower().Contains(postGetAllRequest.SearchTerm.ToLower()) || p.Description.ToLower().Contains(postGetAllRequest.SearchTerm.ToLower()));

        if (postGetAllRequest.Tags is not null)
            query = query.Where(p => p.Tags.Select(t => t.Id).Intersect(postGetAllRequest.Tags).Any());

        query = (postGetAllRequest.SortBy?.ToLower(), postGetAllRequest.SortOrder) switch
        {
            ("votes", SortOrder.Asc) => query.OrderBy(p => p.Votes.Count(v => v.VoteType == VoteType.Upvote) - p.Votes.Count(v => v.VoteType == VoteType.Downvote)),
            ("date", SortOrder.Asc) => query.OrderBy(p => p.CreatedAt),
            ("votes", SortOrder.Desc) => query.OrderByDescending(p => p.Votes.Count(v => v.VoteType == VoteType.Upvote) - p.Votes.Count(v => v.VoteType == VoteType.Downvote)),
